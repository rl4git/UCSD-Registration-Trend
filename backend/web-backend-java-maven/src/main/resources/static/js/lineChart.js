...
function initializeLineChart() {
  ...
  lineChart = new Chart(ctx, {
    ...
    data: {
      labels: [
        "Prior",
        "First Pass Priorities & Seniors Start",
        "First Pass Juniors Start",
        "First Pass Sophmores Start",
        "First Pass First-Year Start",
        "Second Pass Priorities & Seniors Start",
        "Second Pass Juniors Start",
        "Second Pass Sophmores Start",
        "Second Pass First-Year Start",
        "Quarter Start",
        "A Week After Quarter Start",
      ],
      datasets: [
...
function addCourseLine(course, color) {
  ...
  const availableSpotsRatio = course.availableSpots.map(
    (spot, index) => (spot - course.waitlistCount[index]) / course.courseSize
  );
...
function toggleLineVisibility(course, toggleInput, courseTab, color) {
  ...
  const courseId = `${course.courseId} - ${course.professorFirstName} ${course.professorLastName} - ${course.academicQuarter}`;
  const datasetIndex = lineChart.data.datasets.findIndex(
    (ds) => ds.id === courseId
  );
