...
class CourseTab {
  constructor() {
    ...
    this.tabElement.querySelector(".check-btn").onclick = () =>
      this.handleCheck();
    this.tabElement.querySelector(".remove-btn").onclick = () =>
      this.removeTab();
    this.tabElement.querySelector(".hide").onchange = () => this.toggleHidden();
    this.tabElement.querySelector(".overall-display").onchange = () =>
      this.overallDisplay();
  }

  handleCheck() {
    let department = this.tabElement
      .querySelector(".department-input")
      .value.trim();
    let courseId = this.tabElement
      .querySelector(".course-id-input")
      .value.trim();
    department = department.toUpperCase();
    courseId = courseId.toUpperCase();

    if (
      alreadyFetchedCoursesDepartmentId.includes(`${department}-${courseId}`)
    ) {
      alert("The course you entered already been queried.");
      return;
    }

    this.fetchedCourses.forEach((courseComposeIds) =>
      removeCourseLine(courseComposeIds)
    );
    ...
    if (cachedData) {
      if (cachedData.length === 0) {
        ...
        cachedData.forEach((course) =>
          this.fetchedCourses.push(
            `${course.courseId} - ${course.professorFirstName} ${course.professorLastName} - ${course.academicQuarter}`
          )
        );
        this.setOverallCourses(department, courseId, cachedData);
        this.fetchedCourses.push(
          `${this.OverallCourse.courseId} - ${this.OverallCourse.professorFirstName} ${this.OverallCourse.professorLastName} - ${this.OverallCourse.academicQuarter}`
        );
      }
  ...
  overallDisplay() {
    ...
    if (overallToggle.checked) {
      ...
      addCourseLine(
        this.OverallCourse,
        this.tabElement.querySelector(".tab-color-picker").value
      );
    } else {
  ...
  toggleHidden() {
    const hideToggle = this.tabElement.querySelector(".hide");
    const professorTabsContainer = this.tabElement.querySelector(
      ".course-professor-tabs"
    );

    if (hideToggle.checked) {
      // Hide the course-professor-tabs if any hidden-btn is inactive
      const professorTabs = professorTabsContainer.querySelectorAll(
        ".course-professor-tab"
      );
  ...
  showNotFoundMessage() {
    const tabsContainer = this.tabElement.querySelector(
      ".course-professor-tabs"
    );
    ...
    notFoundTab.textContent =
      "Sorry, the course you are looking for does not exist in our dataset.";
  ...
  renderCourses(courses) {
    ...
    courses.forEach((course) => {
      ...
      courseInfo.innerHTML = `
                <span class="professor-name">${course.professorFirstName} ${
        course.professorMiddleName || ""
      } ${course.professorLastName}</span> - 
                ${course.courseId} - ${course.academicQuarter} - Size: ${
        course.courseSize
      }`;
      ...
      toggleInput.addEventListener("change", () =>
        toggleLineVisibility(course, toggleInput, courseTab, colorPicker.value)
      );
  ...
  setOverallCourses(department, courseId, courses) {
    ...
    courses.forEach((course) => {
      ...
      this.OverallCourse.availableSpots = this.OverallCourse.availableSpots.map(
        (value, index) => value + course.availableSpots[index]
      );
      this.OverallCourse.enrolledStudents =
        this.OverallCourse.enrolledStudents.map(
          (value, index) => value + course.enrolledStudents[index]
        );
      this.OverallCourse.waitlistCount = this.OverallCourse.waitlistCount.map(
        (value, index) => value + course.waitlistCount[index]
      );
    });
  }
  ...
  appendToContainer() {
    document
      .getElementById("course-tabs-container")
      .appendChild(this.tabElement);
  }
}
